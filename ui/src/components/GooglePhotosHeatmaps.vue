...
import { loadPhotosSummary, groupYears } from '../services/googlePhotosData';

const isLoading = ref(true);
const error = ref<string | null>(null);
const entries = ref<GooglePhotosSummaryEntry[]>([]);
const years = ref<number[]>([]);
const psychEvents = ref<Record<string, { description: string; prescription?: string[] }>>({});

onMounted(async () => {
  try {
    const data = await loadPhotosSummary('/activity/data/googlephotos.summary.json');
    entries.value = data;
...
<template>
  <div>
    ...
    <div v-else>
      <YearHeatmap v-for="y in years" :key="y" :year="y" :entries="entries" :psych-events="psychEvents" y-field="count" value-label="photos" />
    </div>
  </div>
