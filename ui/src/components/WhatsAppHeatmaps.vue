...
import { loadWhatsAppSummary, groupYears } from '../services/whatsappData';

const isLoading = ref(true);
const error = ref<string | null>(null);
const entries = ref<WhatsAppSummaryEntry[]>([]);
const years = ref<number[]>([]);
const psychEvents = ref<Record<string, { description: string; prescription?: string[] }>>({});

onMounted(async () => {
  try {
    const data = await loadWhatsAppSummary('/activity/data/whatsapp.history.summary.json');
    entries.value = data;
...
<template>
  <div>
    ...
    <div v-else>
      <YearHeatmap v-for="y in years" :key="y" :year="y" :entries="entries" :psych-events="psychEvents" y-field="msgs_sent" value-label="msgs" />
    </div>
  </div>
